...
$$
\Pr(Y_{ij}=1) = \mathrm{logit}^{-1} \left( \sum_{k=1}^{p} \beta_k x_{ijk} \right)
$$

Equivalently:

$$
\log \left( \frac{\Pr(Y_{ij}=1)} {1 - \Pr(Y_{ij}=1)} \right) = \sum_{k=1}^{p} \beta_k x_{ijk}
$$
...
$$
x_{ij,\mathrm{edges}} = 1
$$

## Common-neighbor term

$$
x_{ij,\mathrm{common\_neighbors}} = |N(i) \cap N(j)|
$$

## Transitivity proxy

$$
x_{ij,\mathrm{transitiveties}} = |N(i) \cap N(j)|
$$
...
$$
x_{ij,\mathrm{degree1}} = \begin{cases} 1, & \text{if } d_i = 1 \text{ or } d_j = 1 \\ 0, & \text{otherwise} \end{cases}
$$

## GWESP approximation

$$
x_{ij,\mathrm{gwesp}} = 1 - (1 - e^{-\alpha})^{s_{ij}}
$$
...
$$
x_{ij,\mathrm{nodematch:attr}} = \begin{cases} 1, & \text{if } a_i = a_j \text{ and both values are non-missing} \\ 0, & \text{otherwise} \end{cases}
$$
